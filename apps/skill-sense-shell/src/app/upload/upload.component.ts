  ...
  async uploadFile() {
    ...
    try {
      ...
      if (!userId || !this.selectedFile) {
        console.error('[UPLOAD] Missing userId or file:', { userId: !!userId, file: !!this.selectedFile });
        throw new Error('User not authenticated or no file selected');
      }

      console.log('[UPLOAD] User authenticated:', userId);
      console.log('[UPLOAD] Initiating API call to extract skills...');

      this.apiService.extractFromCV(userId, this.selectedFile).subscribe({
        next: (response) => {
          const duration = ((Date.now() - startTime) / 1000).toFixed(2);
          console.log(`[UPLOAD] ✓ API response received (${duration}s):`, response);

          this.uploadStatus = 'Skills extracted successfully!';
          const skillsFound = response.skillsFound || response.result?.skills?.skills?.length || response.result?.skills?.length || 0;

          this.result = {
            jobId: response.jobId || 'completed',
            status: 'completed',
            skillsFound,
            message: response.message || `Extracted ${skillsFound} skills from your CV`
          };

          this.uploading = false;

          console.log(`[UPLOAD] ✓ Upload completed successfully - ${this.result.skillsFound} skills extracted in ${duration}s`);
          console.log('[UPLOAD] Full result:', this.result);
        },
        error: (err) => {
          const duration = ((Date.now() - startTime) / 1000).toFixed(2);
          console.error(`[UPLOAD] ✗ API call failed (${duration}s):`, err);
      ...
      const duration = ((Date.now() - startTime) / 1000).toFixed(2);
      console.error(`[UPLOAD] ✗ Upload failed (${duration}s):`, err);
